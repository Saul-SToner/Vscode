...
run_guided_grating_demo.py 支线最小示例入口
```
...
当前 `teaching_main_branch_catalog.json` 已明确写入：
...
1. 厚度反演不是当前教学平台的一部分
2. 前端不应把反演入口展示在教学平台主菜单中
3. 反演能力仍可保留给研究模式、内部工具或后续扩展使用
...
5. 当前求解器仍是明确标注的占位模型，不用于正式物理论证

推荐当前只把它当作：

- 支线工程骨架
- 导出链路验证
- 后续 COMSOL / RCWA 接口预留层
